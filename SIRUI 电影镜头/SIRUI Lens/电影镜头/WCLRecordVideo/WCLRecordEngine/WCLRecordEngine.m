...
@interface WCLRecordEngine ()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureAudioDataOutputSampleBufferDelegate, CAAnimationDelegate> {
    ...
    CGFloat distortionRatio;//电影镜头校正录制完成畸变
    CGFloat ratio;          //展示界面调整
}
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.maxRecordTime = CGFLOAT_MAX;//默认无限大时间
        DeviceType type = [SRDeviceUtils deviceType];
        switch (type) {
            case iPhone_XS_Max:
                distortionRatio = 1.29;
                ratio = 1.08;
                break;
            case iPhone_XS:
                distortionRatio = 1.29;
...
//获取视频的方向
- (void)getCaptureVideoOrientation {
    ...
    self.videoConnection.videoOrientation = result;
}
...
- (AVCaptureVideoPreviewLayer *)previewLayer {
    if (_previewLayer == nil) {
        ...
        preview.frame = CGRectMake((KScreenWidth - KScreenWidth/ratio)/2.0, 0, KScreenWidth/ratio, KScreenHeight);
