...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.cellName = [[UILabel alloc] initWithFrame:CGRectMake(LEFT_WIDTH, 0, frame.size.width/3*2, frame.size.height)];
        _cellName.textColor = [UIColor whiteColor];
        _cellName.font = [UIFont systemFontOfSize:FONT_SIZE];
        [self addSubview:_cellName];
        
        self.cellView = [[UIView alloc] initWithFrame:CGRectMake(frame.size.width/3*2, 0, frame.size.width/3, frame.size.height)];
