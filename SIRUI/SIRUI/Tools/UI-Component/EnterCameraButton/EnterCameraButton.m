...
@implementation EnterCameraButton
...
- (void)createButtonWithFrame:(CGRect)frame {
    ...
    {
        // 按住按钮后没有松手的动画
        [self.button addTarget:self action:@selector(buttonTouchDownAndDragEnter)
...
- (void)buttonTouchDownAndDragEnter {
    ...
    [UIView animateWithDuration:(self.toEndDuration <= 0 ? TIME_END_DURATION : self.toEndDuration) animations:^{
        ...
        if (finished == YES) {
            ...
            if (self.delegate && ([self.delegate respondsToSelector:@selector(finishedEventByEnterCameraButton:)])) {
                
                [self.delegate finishedEventByEnterCameraButton:self];
            }
        }
    }];
}
